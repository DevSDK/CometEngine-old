...
public class JPEGImageReader extends JPEG {
	...
	final static int[] ZIGZAG = new int[] { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5, 12, 19, 26, 33, 40,
	...
	private JPEGImage readImpl() throws IOException {
		...
		try {
			...
			{
				throw new IOException("Error in JPEG stream; expected SOI segment marker but found: "
						+ Integer.toString(nextSegment, 16));
			}
			...
			do {
				...
				if ((nextSegment >> 8) != 255) {
					throw new IOException("Error in JPEG stream; expected segment marker but found: "
							+ Integer.toString(nextSegment, 16));
				}
				...
				case EOI:
					break;
				case SOF2:
					throw new IOException("JPEG Progressive images not supported.");
				default:
					mBitStream.skip(mBitStream.readShort() - 2);
					break;
				}
	...
	private JPEGImage readRaster() throws IOException {
		...
		JPEGImage image = new JPEGImage(mFrameSegment.getWidth(), mFrameSegment.getHeight(), maxSampling,
				mDensitiesUnits, mDensity, mFrameSegment.getComponentCount());
			...
			for (int y = 0, index = 0; y < numVerMCU; y++) {
				for (int x = 0; x < numHorMCU; x++, index++) {
					for (int component = 0; component < numComponents; component++) {
						...
						// mQuantizationTables[c.getQuantizationTableId()].getTable();
						...
						for (int cy = 0; cy < sampling.y; cy++) {
							for (int cx = 0; cx < sampling.x; cx++) {
								if (!readDCTCofficients(dctCoefficients, component)) {
									if (mcuCounter == 0) {
										throw new IOException(
												"Error reading JPEG stream; Failed to decode Huffman code.");
									}
									image.setDamaged();
									return image;
								}
					...
					if (mRestartInterval > 0 && (((index + 1) % mRestartInterval) == 0)) {
						if (index < numHorMCU * numVerMCU - 1) // Don't check
																// restart
																// marker when
																// all MCUs are
																// loaded
						{
							...
							if (restartMarker != 0xFFD0 + restartMarkerIndex) {
								throw new IOException("Error reading JPEG stream; Expected restart marker "
										+ Integer.toHexString(0xFFD0 + restartMarkerIndex));
							}
