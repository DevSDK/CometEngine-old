...
public class CESprite2D extends CERenderableObject implements CEBound2D {
	...
	public static CESprite2D Create(String filename) {
		CESprite2D sprite = new CESprite2D(filename);
		return sprite;
	}

	private CESprite2D(String filename) {
	...
	private void UpdateContentSize() {
		ContentSize.x = texture.getData().getWidth();
		ContentSize.y = texture.getData().getHeight();
	}
