package com.CometEngine.Model.obj.parser;
...
public class Parse {
	...
	private final static String OBJ_OBOJECT_NAME_H = "O";
	...
	private ByteBuffer ObjRawData;

	public Parse(BuilderInterface builder, ByteBuffer RawData, String filename)
			throws FileNotFoundException, IOException {
		ObjRawData = RawData;
		this.builder = builder;
		builder.setObjFilename(filename);
		FileLocation = getFileLocation(filename);
		parseObjFile(RawData);

		builder.doneParsingObj(filename);
	}

	private void parseObjFile(ByteBuffer rawData) throws IOException {
		int lineCount = 0;
		BufferedReader bufferedReader = null;

		bufferedReader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(rawData.array())));
	...
	private void parseMtlFile(String mtlFilename) throws FileNotFoundException, IOException {
		...
		ByteBuffer buf = CEFileUtil.getInstence().ReadResurceDirectoryToSync(FileLocation + mtlFilename);

		bufferedReader = new BufferedReader(new InputStreamReader(new ByteArrayInputStream(buf.array())));
		...
		while (true) {
			...
			} else if (line.startsWith(MTL_MAP_BUMP)) {
