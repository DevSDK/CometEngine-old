...
public final class Matrices {
	...
	public static final Mat4 perspective(final float fovy, final float aspect, final float zNear, final float zFar) {
		...
		return new Mat4((2f * zNear) / (right - left), 0f, 0f, 0f, 0f, (2f * zNear) / (top - bottom), 0f, 0f, 0f, 0f,
				-(zFar + zNear) / (zFar - zNear), -1f, 0f, 0f, -(2f * zFar * zNear) / (zFar - zNear), 0f);
	}
	 ...
	 * @return
	 */
	public static final Mat4 frustum(final float left, final float right, final float bottom, final float top,
			final float nearVal, final float farVal) {
		...
		return new Mat4(m00, 0f, 0f, 0f, 0f, m11, 0f, 0f, m20, m21, m22, m23, 0f, 0f, m32, 0f);
	}
	...
	public static final Mat4 lookAt(final Vec3 eye, final Vec3 center, final Vec3 up) {
		...
		return new Mat4(s.x, u.x, -f.x, 0f, s.y, u.y, -f.y, 0f, s.z, u.z, -f.z, 0f, -s.dot(eye), -u.dot(eye),
				f.dot(eye), 1f);
	}
	 ...
	 * @return
	 */
	public static final Mat4 ortho(final float left, final float right, final float bottom, final float top,
		...
		return new Mat4(m00, 0f, 0f, 0f, 0f, m11, 0f, 0f, 0f, 0f, m22, 0f, m30, m31, m32, 1f);
	}
	...
	public static final Mat4 ortho2d(final float left, final float right, final float bottom, final float top) {
		...
		return new Mat4(m00, 0f, 0f, 0f, 0f, m11, 0f, 0f, 0f, 0f, m22, 0f, m30, m31, 0f, 1f);
	}
	 ...
	 */
	public static Mat4 rotate(final float phi, float ax, float ay, float az) {
		...
		float m00 = (float) (rcos + x * x * (1 - rcos));
		float m01 = (float) (z * rsin + y * x * (1 - rcos));
		float m02 = (float) (-y * rsin + z * x * (1 - rcos));

		float m10 = (float) (-z * rsin + x * y * (1 - rcos));
		float m11 = (float) (rcos + y * y * (1 - rcos));
		float m12 = (float) (x * rsin + z * y * (1 - rcos));

		float m20 = (float) (y * rsin + x * z * (1 - rcos));
		float m21 = (float) (-x * rsin + y * z * (1 - rcos));
		float m22 = (float) (rcos + z * z * (1 - rcos));
